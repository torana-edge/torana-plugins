...
func TestManifestPermissionSetExact(t *testing.T) {
	...
	sort.Strings(got)
	want := []string{"env.cache_get", "env.cache_set", "env.emit_metric", "env.log", "env.meta_get", "env.meta_set", "env.plugin_config", "ir.messages.write.assistant", "ir.messages.write.system", "ir.messages.write.tool", "ir.messages.write.user", "ir.stream.write", "ir.tool_results.write", "ir.tools.write"}
	if len(got) != len(want) {
