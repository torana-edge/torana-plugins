...
func TestPluginConfigRefusalDoesNotApplyDefaultPolicyAndRetrySucceeds(t *testing.T) {
	...
	h.StubHostCall("env.plugin_config", func(string) (string, error) {
		...
		return sdktest.HostResultValue([]byte(`{"conversations":"conv-1","warm_for_minutes":12}`)), nil
	})
	h.Run(func() {
		cfg, err := loadConfig()
		...
		if cfg.any() || cfg.WarmForMinutes != 0 {
	...
	h.Run(func() {
		...
		if !cfg.warms("conv-1") || cfg.WarmForMinutes != 12 {
...
func TestPluginConfigRefusalStopsHookBeforeWarmSideEffects(t *testing.T) {
	h := newHarness(t)
	calls := 0
	h.StubHostCall("env.plugin_config", func(string) (string, error) {
		calls++
