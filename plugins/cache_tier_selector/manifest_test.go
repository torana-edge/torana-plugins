...
func TestManifestPermissionSetExact(t *testing.T) {
	...
	sort.Strings(got)
	want := []string{"env.cache_policy", "env.host_call.torana_plugin_counter", "env.log", "env.now", "env.plugin_config", "env.state_get", "env.state_keys", "env.state_set", "ir.cache_control.write"}
	if len(got) != len(want) {
